...
//        }
    ...
    public function tabs() {                                                    // T a b s      A r r a y
        ...
        $subtabs['new-admin'] = array( 
                            ...
                            , 'title' =>  $icon . __('New' ,'booking') . ' (' . __('admin' ,'booking') . ')'        // Title of TAB    
                            , 'page_title' => __('Emails Settings', 'booking')  // Title of Page   
                            , 'hint' => __('Customization of email template, which is sending to Admin after new booking' ,'booking')   // Hint    
                            , 'link' => ''                                      // link
                            , 'position' => ''                                  // 'left'  ||  'right'  ||  ''
                            , 'css_classes' => ''                               // CSS class(es)
...
//debuge($init_fields_values);        
            ...
            <form  name="<?php echo $submit_form_name; ?>" id="<?php echo $submit_form_name; ?>" action="" method="post" autocomplete="off">
                ...
                <div class="metabox-holder">
                    ...
                    <?php 
                        ...
                        wpbc_open_meta_box_section( $submit_form_name . 'general', __('Email is sending to Admin after creation of booking.', 'booking')   );
    ...
    public function update_actions(  ) {
        ...
        if ( $_POST['form_action'] == 'test_send' ) {                           // Sending test  email
            ...
            $email_fields = get_bk_option( 'wpbc_email_' . $selected_email_name );
        ...
        if ( $_POST['form_action'] == 'create' ) {                              // Create
            ...
            $email_name = wpbc_get_slug_format_4_option_name( $email_title );
            
            $wpbc_email_tpl_names = get_bk_option( 'wpbc_email_tpl_names' );
            if ( empty( $wpbc_email_tpl_names ) )  $wpbc_email_tpl_names = array();
            
            
            if ( empty($email_name) || isset( $wpbc_email_tpl_names[ $email_name ] ) ) {      // Error
                wpbc_show_message ( __('Email template has not added.', 'booking'), 5 , 'error' );  
                return false;                
            }
            
            $wpbc_email_tpl_names[ $email_name ]= stripslashes( $email_title );
            
            update_bk_option( 'wpbc_email_tpl_names', $wpbc_email_tpl_names );
            
            wpbc_show_message ( __('Email template added successfully', 'booking'), 5 );                                               // Show Save message
            
            $redir = esc_url( add_query_arg( array('email_template' => $email_name ), html_entity_decode( $this->getUrl() ) ) );       
            
            wpbc_reload_page_by_js( $redir );
        ...
        if ( $_POST['form_action'] == 'delete' ) {                              // Delete
            ...
            if ( ! isset( $wpbc_email_tpl_names[ $email_name ] ) ) {            // Error
                wpbc_show_message ( __('Email template does not exist.', 'booking'), 5 , 'error' );  
                return false;                
            } 
            
            unset($wpbc_email_tpl_names[ $email_name ]);                        // Remove Email  name from list of email names
            update_bk_option( 'wpbc_email_tpl_names', $wpbc_email_tpl_names );
            
            delete_bk_option( 'wpbc_email_' . $email_name );                  // Delete Email Template
            
            wpbc_show_message ( __('Email template deleted successfully', 'booking'), 5 );                                     // Show Save message
...
//          ) return;
        ...
        $js_script .= "     var wpbc_email_from = jQuery('#new_admin_from').val();";    // from@wpbookingcalendar.com 
